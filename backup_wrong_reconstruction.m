...
[Zernike_Phase,Terms]=ZernikeModal(Fx*10,Fy*10);       %Modal Phase Reconstruction
...
if sensor_type == 0
    ...
    figure;
    surf(phase_CER);title(['NVWCER Unwrapped ' ' SNR=' num2str(SNR0) 'N=' num2str(NN)]);colorbar; colormap gray;annotation('textbox',dim,'String',str,'FitBoxToText','on');
    saveas(gcf,'NVWCER_Unwrapped_sensor.fig','fig'); 

    figure;
    surf(phase_zonal);title(['Zonal Unwrapped ' ' SNR=' num2str(SNR0) 'N=' num2str(NN)]);colorbar; colormap gray; 
    saveas(gcf,'Zonal_Unwrapped_sensor.fig','fig'); 

    figure;
    surf(Zernike_Phase);title(['Zernike Fit Unwrapped ' ' SNR=' num2str(SNR0) 'N=' num2str(NN)]);colorbar; colormap gray; 
    saveas(gcf,'Zernike_Fit_Unwrapped.fig','fig'); 

    figure;
    imagesc(mod(phase_CER,2*pi));title(['NVWCER Wrapped ' ' SNR=' num2str(SNR0) 'N=' num2str(NN)]);colorbar; colormap gray; 
    saveas(gcf,'NVWCER_Wrapped_sensor.fig','fig'); 

    figure;
    imagesc(mod(phase_zonal,2*pi));title(['Zonal Wrapped ' ' SNR=' num2str(SNR0) 'N=' num2str(NN)]);colorbar; colormap gray; 
    saveas(gcf,'Zonal_Wrapped_sensor.fig','fig'); 

    figure;
    imagesc(mod(Zernike_Phase,2*pi));title(['Zernike Fit Wrapped ' ' SNR=' num2str(SNR0) 'N=' num2str(NN)]);colorbar; colormap gray; 
    saveas(gcf,'Zernike_Fit_Wrapped.fig','fig');
    
    
    figure;
    surf(WFSzernikefit);title(['Zernike Fit based on WFS ' 'N=' num2str(a)]);colorbar; colormap gray; 
